...
export async function match(req, res, next) {
  try {
    ...
    // Stage 1.2: Merging candidates (UNION on field archetypeId) from both queries
    ...
    const resultIds = await fetchBipartiteMatchResults([queryVectorA, queryVectorB], candidateIds, topN);
    const resultIdsSet = new Set(resultIds);
    // Get full data for top N candidate ids from candidates array
    const results = candidates.filter(c => resultIdsSet.has(String(c.id)));
    // Format results as needed for response and send back
    return res.status(200).json({
      error: false,
      matchesFoundCount: resultIds.length,
      results: results.map(formatResults)
    });
  } catch (error) {
